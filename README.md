...
| 验证 | **511 项自动回归断言** + 压力基准 + 威胁基准 + 浸泡基准 + 追击基准，八个模式全绿 |
| 素材 | 8 段程序合成音频，0 个第三方文件 |
...
| Boss 转阶段（全屏闪白 + 血条换色） | Boss 二阶段（螺旋弹幕） |
| --- | --- |
| ![Boss 转阶段](screenshots/boss-phase.png) | ![Boss 二阶段](screenshots/boss-spiral.png) |
...
| 贯穿光束（聚焦型） | 拦截弹（专属光环） |
| --- | --- |
| ![贯穿光束](screenshots/beam.png) | ![拦截弹](screenshots/interceptor.png) |
...
- **开局选战机**。形态**不进抽卡池**：不稀释 16 项能力的抽取，也不会"什么都拿一点"导致输出与形状同时膨胀，还避开了"中途改武器会让前几级投资作废"。选择会记住，结算页有"更换战机"回开始界面。当前三台，**开火方式完全不同**：
  - **标准型** — 平行弹幕。按住开火射出竖直平行的弹道墙，每级一条车道，弹幕墙能靠拦截弹清掉敌弹；青蓝色短弩形弹体。
  - **游隼型** — **只出导弹**。按下开火发射跟踪弹（单发目标、带尾翼的弹形、暖色），**锁定射程 420 像素**——够不着的敌机不会被锁定，所以这台战机必须主动上前，和聚焦型是同一套取舍。它不出普通子弹，因此火力增援 / 侧翼炮 / 穿透弹 / 拦截弹对它都不提供（前三个没变化，拦截弹会让导弹半路被敌弹消耗掉）。
  - **聚焦型** — 贯穿光束，**按开火打出一串激光**（一串 0.45 秒、串间 0.14 秒），不再是开机就常驻的光环；射速强化缩短的是串间间隔。敌机进柱即在下次结算时被击毁、没有飞行时间，代价是**覆盖窄**（每条 10 像素）与**射程短**（340 像素）——站着不动既够不到屏幕顶部，也够不到 Boss。
  - **每台战机只用自己的弹体**（形状 + 配色都是它自己的），而**判定盒三台完全一致**——视觉可以有个性，命中盒不能有个性。
- **拦截弹有自己的外观**：拿到之后每颗子弹会带一圈光环（而且判定盒不变大）。以前它只是一个开关，拿完之后画面上毫无变化。
...
- **Boss 有两个阶段，二阶段换的是弹幕而不是速度。** 血量掉到 50% 时：停住横向巡航，扇形齐射改成**螺旋弹**——一组弹均分整圈射出，每轮整体再转一个固定角，所以相邻几轮叠起来才看得出在转；弹速比扇形慢，留出"找缝隙钻"的时间。转阶段有 0.5 秒的仪式：**全屏闪白 + 屏幕震动 + 专属音效**（上行抖颤，与登场的下行音效正好一对），血条文字与配色同时变暖。**"停一下、闪一下、换弹幕"是刻意的**——不声不响地换一套弹幕，玩家只会以为弹幕变密了，而不会觉得"它变招了"。
...
├── assets/
│   ├── audio/                8 段程序合成的 WAV（含循环背景音乐与 Boss 转阶段音效）
...
```bash
python tools/validate_project.py smoke     # 494 项回归断言
python tools/validate_project.py visual    # 生成 10 张实机截图
