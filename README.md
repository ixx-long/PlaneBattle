...
```bash
python tools/validate_project.py smoke     # 367 项回归断言
python tools/validate_project.py visual    # 生成 6 张实机截图
...
- **对象池：实测后决定不做。** 最坏情况峰值并发子弹 88 发、峰值 Actors 约 105 个、平均帧 8.2 ms。对象周转比"需要池化"的量级低两个数量级，而池化必须重做幂等锁、跨局票据、统一清场三处最要命的约定。**重新评估的触发条件是压力基准失败**，不是"以后再说"。
