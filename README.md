...
| 物理 | 120 Hz |
| 规模 | 9 个脚本 / 8 个场景 / 16 项能力 / 5 套波次编队 |
| 验证 | **367 项自动回归断言** + 压力基准 + 威胁基准，六个模式全绿 |
...
| 暂停与设置 |
| --- |
| ![暂停与设置](screenshots/pause.png) |

> 截图由 `tests/VisualTest.gd` 在真实图形模式下渲染生成，不是设计稿。
...
- **开局选战机**。形态**不进抽卡池**：不稀释 16 项能力的抽取，也不会"什么都拿一点"导致输出与形状同时膨胀，还避开了"中途改武器会让前几级投资作废"。选择会记住，结算页有"更换战机"回开始界面。当前两台：
  - **标准型** — 平行弹幕。每级一条竖直车道，弹幕墙能靠拦截弹清掉敌弹。
  - **游隼型** — 追踪弹。子弹向**近距离**（110 像素内）的最近敌机转向，更远的敌机照直飞。代价就是它的身份：**打不到远处，只能近程拦阻**。
...
```bash
python tools/validate_project.py smoke     # 406 项回归断言
python tools/validate_project.py visual    # 生成 7 张实机截图
...
**威胁基准**回答的是"后期还有没有威胁"：把玩家固定在屏幕下方且**全程不动**，量敌弹通过率与**每架敌机开了几枪**。基准固定了随机种子——不固定时同一个量在 8~19 之间跳，门槛就只能定得极松，而守不住东西的下限等于没有下限。**它遍历全部战机**，每台都必须达标：形态把平衡面翻了一倍，靠手调守不住。当前实测（难度 30 的"到达/秒"，门槛 12）：标准型 **19.83**、游隼型 **15.17**。

**追踪弹的平衡过程值得一读**，因为最初的设计被数据直接推翻了：直觉上"追踪 = 命中率高"，想用转向速率当成本——实测**完全无效**（转向降到 1.0 rad/s，击毁率仍 92~96%、敌方弹幕被打到 **0**）。原因是结构性的：敌机朝玩家下来，射程不受限时追踪一定撞得上，而满配每秒 148 发、在飞约 170 发，上半屏会被整个清空。真正的支点是**锁定射程**。实测曲线：射程 70 → 15.17、110 → **15.33**、300 → 6.50、600 → 5.17、不限制 → ~0。断点很陡，所以它是平衡参数而不是手感参数。

**浸泡基准**回答的是压力基准答不了的那个问题——"长期局会不会越打越卡"。满配连打 **60 秒**（压力基准的十倍）后对比前后半段：**前半 8.33 ms → 后半 8.33 ms**，实体峰值 111 → 105，停火后残留 0，对象净增 9。它同样固定了随机种子，否则前后对比会被出生位置的随机差异淹没。

## 一些设计取舍

这些决定都有实测或推导支撑，写在文档里以免后来人凭直觉重做一遍。

- **对象池：实测后决定不做，现已用长时基准结案。** 最坏情况峰值并发子弹 88 发、峰值 Actors 约 105 个、平均帧 8.2 ms；再叠加 60 秒浸泡——**前后半段帧时间完全相同（8.33 → 8.33 ms）、实体峰值不增长、对象净增 9**，说明对象周转没有随时间退化。而池化必须重做幂等锁、跨局票据、统一清场三处最要命的约定。**重新评估的触发条件是压力基准或浸泡基准失败**，不是"以后再说"。
...
跑完会在 `outputs/` 下生成三样东西：**完整开发文档**（Markdown + 离线可读的 HTML，约 5700 行，逐字嵌入全部源码、场景节点树与坐标、碰撞层与遮罩表、波次与难度参数表、常见错误排错表）、**验证报告**（本次运行的完整回归日志）、以及给玩家的**源码 ZIP**。三者都从同一份源码重建，不会互相说谎。
