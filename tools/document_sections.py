...
- **开局选战机**：开始界面显示可选战机，选定后整局沿用，并记进存档（下次启动仍是它）。**形态刻意不进抽卡池**——不稀释 16 项能力的抽取，也不会出现"什么都拿一点"导致覆盖形状与输出同时膨胀；同时它避开了"中途改武器会让前几级投资作废"的问题。当前三台：
  - **标准型**：平行弹幕，基线。
  - **游隼型**：**主弹幕照直飞，另按节奏射出全屏锁定的追踪导弹**（更大、暖色，一眼可辨）。
  - **聚焦型**：**贯穿光束**，出膛即中——光柱（`scenes/Beam.tscn`）是一条持续存在的竖直致命线，敌机一进柱就在下一次结算时被击毁，**没有飞行时间**；代价是**覆盖窄（每根 10 像素）且射程短（340 像素，不再直达战斗区顶边）**。
...
**它与其余实体的本质区别是：它持续存在，而且几何每帧都在变。** 子弹是“生成 → 飞 → 命中或出界 → 自己释放”，光柱则从开局起一直在屏幕上，长度随玩家上下移动而伸缩，条数与横向位置则**照抄子弹车道的算法**——`_lane_offsets()` 是子弹与光柱共用的唯一来源，所以“火力增援/侧翼炮”在这台战机上含义完全相同，两台战机的差别只剩“瞬间命中 vs 飞行时间”和“覆盖窄 + 射程短 vs 弹幕墙”。
