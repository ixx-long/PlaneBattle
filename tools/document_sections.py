...
- **开局选战机**：开始界面显示可选战机，选定后整局沿用，并记进存档（下次启动仍是它）。**形态刻意不进抽卡池**——不稀释 16 项能力的抽取，也不会出现"什么都拿一点"导致覆盖形状与输出同时膨胀；同时它避开了"中途改武器会让前几级投资作废"的问题。当前两台：
  - **标准型**：平行弹幕，基线。
  - **游隼型**：**主弹幕照直飞，另按节奏射出全屏锁定的追踪导弹**（更大、暖色，一眼可辨）。
  第一版刻意**只改武器行为与外形**，不碰移速/生命/冷却：两台战机各自换一套武器，需要验证的组合已经翻倍，再叠加数值取舍，出问题时就分不清是形态还是数值造成的。
...
├── ShipButton2 (Button) [309,218 → 433,272; 初始 Hidden；加第三台战机时自动排开]
...
**开始界面的战机选择**：三个按钮的位置**按实际台数现算**（`show_ship_choice()` 里按可用宽度均分），不写死三档坐标——否则加第四台时新按钮会叠在旧的上。选中态需要**明显可辨**：主题的 pressed 与 normal 太接近，所以给按钮单独挂了深青底白字的 `pressed` / `hover_pressed` 样式；第一版只靠 `ButtonGroup` 的默认按下态，截图里两台看起来一模一样。
...
**`res://tests/SoakTest.gd` 是长时浸泡基准，回答“长期局会不会越打越卡”**。压力基准只测 6 秒，能证明“瞬时负载在预算内”，证明不了“时间长了会不会变差”。它让满配玩家连续打满 60 秒（压力基准的十倍），对比**前后半段的帧时间**与**开头/结尾的实体峰值**，并断言停火后残留为 0、对象净增有上限。实测：前半 8.33 ms → 后半 8.33 ms（毫无漂移）、实体峰值 111 → 105、残留 0、对象净增 9。**它的边界也写清楚**：60 秒仍不是完整的一局（真人一局 2~5 分钟），这条基准没有覆盖到那么长。它同样固定了随机种子，否则前后半段的对比会被出生位置的随机差异淹没。
...
| 两台战机的按钮看不出哪台被选中 | 主题的 pressed 与 normal 样式太接近 | 给按钮单独挂深青底白字的 `pressed`/`hover_pressed` 样式；只靠 ButtonGroup 的默认按下态在截图里看不出区别 |
