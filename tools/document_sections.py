...
`assets/` 是资源目录，字体/按钮主题在 ui_theme.tres 中；飞机与子弹由 Polygon2D 绘制；背景由 ColorRect/Line2D 组成。`assets/audio/` 是八个 WAV 音频（总长约 11 秒、469 KB），**不是下载来的素材，而是由 `tools/generate_audio.py` 用 Python 标准库合成出来的**——固定随机种子，任何人重跑都会得到逐字节相同的文件，因此不存在授权问题。要换成正式音频，直接覆盖同名文件即可，游戏代码不需要改。Godot 首次导入生成的 `.godot/` 与 `.gd.uid` 不必手写；不要把缓存作为必需源码。

**关于“所有代码都能复制”的边界**：本文逐字嵌入的是全部文本资源（`.gd`、`.tscn`、`.tres`、`project.godot`），共 21 个文件。八个 WAV 是二进制，无法以文本形式粘贴——它们随交付 ZIP 一起提供，源文件也保留了生成脚本 `tools/generate_audio.py`（该脚本随 ZIP 附在 `tools/` 下），需要时重跑即可得到完全相同的音频。
...
3. **自带攻击节奏与两个阶段**：入场先降到 `hold_y`，再进入横向巡航并按 `ShootTimer` 齐射扇形弹幕（一阶段）；血量掉到 `phase_two_ratio`（默认 50%）时切二阶段。入场阶段不移动也不开火，那段时间是给玩家的准备。

**二阶段换的是弹幕，不是速度。** 这是它和"把一阶段调快一点"最本质的区别——单纯加速只是同一个形状更密，玩家不会觉得敌人变了；换形状才会。

- **停巡航**：二阶段不再横向移动。位置固定下来，"螺旋"才看得出是在转（一边漂一边转，观感只是"某处有弹在飞"）。
- **扇形 → 螺旋**：每轮把 `spiral_arms` 发弹**均分整圈**射出，并且**每轮整体再转一个固定角**（`spiral_step_degrees`）。"螺旋"的全部秘密就在这个步长里：单看任何一轮都只是个正多边形，只有把相邻几轮叠起来才看得出在转——所以步长与发射间隔必须一起调，步长太小像静止的多边形、太大又变成乱射。发射点是**机体中心**而不是炮口，径向弹幕从核心冒出来才像"转"。弹速刻意比扇形慢，留出"找缝隙钻"的时间。
- **转阶段的仪式**：`transition_seconds`（默认 0.5 秒）之内**停火也停巡航**，同时由 Main 做三件事——全屏闪白（HUD 的 `PhaseFlash`，与受伤红闪分成两层节点，语义不同、也不该互相打断补间）、屏幕震动、专属音效（`phase.wav`：上行的抖颤锯齿，与登场的下行音效正好构成一对）。血条文字与配色也一起变暖。
  **这半秒是故意留的**：不声不响地换一套弹幕，玩家只会觉得"弹幕忽然变密了"，而不会觉得"它换了打法"。变招需要一个句读。
- **阶段只升不降**：`phase` 是"这一场发生过什么"，不是"当前血量落在哪一段"，所以挨了跨过阈值的那一刀之后再怎么打也不会退回一阶段、更不会重复播一次仪式。血条上的"二阶段"标记与血量写在**同一处**（`HUD._refresh_boss_label()`）——各写各的会互相覆盖，第一版就是这么被截图抓到的：标好"二阶段"，下一刀掉血就把它冲掉了。
...
Boss 的两种弹幕都需要带角度的敌弹，因此 `EnemyBullet` 增加了 `direction`（默认正下方，普通敌机行为完全不变）。弹体图形是上下左右对称的六边形，所以不按方向旋转——转了也看不出来。**螺旋弹有朝正上方飞的那几发，因此 `EnemyBullet` 的出界判定补上了顶边**：早期只判了下边与左右，向上飞的弹会一直飞、永远不释放。
...
| 血条上的"二阶段"标记一挨打就没了 | 阶段标记与血量各写各的：`set_boss_phase()` 写一次文字，下一次掉血的 `update_boss()` 又把它整行覆盖 | 两件事**在同一处拼接**（`HUD._refresh_boss_label()`），阶段与血量都存成 HUD 的状态。**这条是截图抓出来的，断言当时全绿**——"标了字"和"字一直在"是两回事 |
| Boss 二阶段测试里"跨过阈值那一刀"没触发 | 断言的触发点用 `ceil(上限 × 比例)` 算，而代码判的是"血量 > 上限 × 比例"；`max_hp` 是奇数时两者差 1 点 | 断言的边界必须与代码**同口径**（用 `floor`）。阈值类断言的第一件事就是确认自己算的是同一个点 |
