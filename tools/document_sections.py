...
- **操作**：WASD 或方向键移动；按住空格连续射击；按住 **Shift** 进入低速模式；**Esc** 暂停；点击开始；结束后点击重新开始或按 R；升级时按数字键 1/2/3 选能力。待开始界面也支持 R；进行中 R 不重开，避免误触。
- **开局选战机**：开始界面显示可选战机，选定后整局沿用，并记进存档（下次启动仍是它）。**形态刻意不进抽卡池**——不稀释 16 项能力的抽取，也不会出现"什么都拿一点"导致覆盖形状与输出同时膨胀；同时它避开了"中途改武器会让前几级投资作废"的问题。当前三台：
  - **标准型**：平行弹幕，基线——按住开火射出竖直平行的弹道墙。
  - **游隼型**：**只出导弹**。按下开火发射跟踪弹（单发目标、带尾翼的弹形、暖色），**锁定射程 420 像素**：够不着的敌机不会被锁定，所以它和聚焦型一样必须主动上前。它不出普通子弹，因此火力增援 / 侧翼炮 / 穿透弹 / 拦截弹对它都不提供。
  - **聚焦型**：**贯穿光束，按开火打出一串激光**（一串 0.45 秒、串间 0.14 秒），不再是开机就常驻的光环。敌机进柱即在下次结算时被击毁、没有飞行时间；代价是**覆盖窄（每根 10 像素）且射程短（340 像素，够不到屏幕顶部，也够不到站在底部时的 Boss）**。
  三台刻意**只改武器行为与外形**，不碰移速/生命/冷却：每台各换一套武器，需要验证的组合已经翻倍，再叠加数值取舍，出问题时就分不清是形态还是数值造成的。
- **"按开火才发射"是三台共用的规则，而它曾经不是**：早期版本里光柱开机就常驻、导弹按自己的节奏自动发射——两者都不需要玩家按开火，真人反馈"不像武器，像光环"。现在三台都只由开火请求驱动（见 `Main._on_player_shoot_requested` 的分派），并且各自有自己的节奏限制：直弹幕受射击冷却限制、导弹受发射冷却限制、激光受串长与串间间隔限制。
- **导弹战机为什么必须有锁定射程**：导弹是**必中**武器，只用"发射射速"这一个旋钮时，它只能在两个坏结果之间二选一——射速高到前场能打够（追击基准达标），静态威胁基准就会崩（D30 只剩 3.5 发/秒到达，因为全屏锁定把屏幕清空了）；射速压低，威胁回来了，前场又打不够（60% 对标准型 80%）。**射程决定"能打到多远"，射速只决定"打得有多快"，缺了前者就没有中间地带。** 收到 420 之后两条基准同时留有余量，而它换来的代价与聚焦型完全一致：必须主动上前。
- **拦截弹的语义是"我的弹幕墙能挡子弹"，不是"我的任何武器都能挡子弹"**：导弹全屏锁定、必然穿越整片敌弹空域飞向目标，一旦让导弹也拦截，它就会在半路被敌弹消耗掉——满配实测击毁率从 73% 掉到 15%，导弹成了"打敌弹的磁铁"。所以导弹**不继承**拦截弹，而这台战机也**不提供**拦截弹：一个"看起来有用、其实在拆自己台"的选项，比没有这个选项更糟。
- **光束的两项代价缺一不可，第二项是真人反馈"太赖皮"之后补的**：第一条是覆盖窄（柱外的敌机完全不受影响）；第二条是**射程**——光柱只延伸到玩家上方 340 像素处。为什么必须有第二条：出膛即中意味着**不需要预判**（子弹必须提前约 0.5 秒打在那个位置，光柱是"现在指到谁谁就没"），而全屏射程意味着玩家可以**永远待在屏幕底部**把上半屏扫干净、还能站在安全距离把 Boss 融化。量化之后最刺眼的一个数：同样是站在屏幕底部不动，光柱对 Boss 有 **141.8 发/秒**的输出，而子弹系在同一个位置只有约 **72/秒**——Boss 战 5 秒结束、零风险。加上射程上限之后，这个数字变成 **0**：想打 Boss 就必须主动上前（实测 8.5 秒击破，与另外两台的 8.0 秒同级）。**射程这个数也是量出来的**：放宽到 380 时只需往上挪十来个像素就能碰到 Boss，那条限制就退化成装饰了。
- **光束战机没有子弹**：输出全部由光柱承担，因此"弹速强化"在这台战机上映射为**充能更快**（缩短结算间隔 `beam_tick_interval`），而不是一个选了没变化的死选项；“拦截弹”表现为**光柱击落柱内敌弹**；**“穿透弹”则确实没有意义**（光柱本来就穿透整列敌人），于是由 `is_upgrade_offered()` 直接不提供——给一个“选了没效果”的选项比不给更糟。这三条各自都有回归断言。
- **追踪导弹为什么不做进主弹幕**：这条结构是被数据逼出来的，两版设计都被推翻。①**每发子弹都追踪**：基准实测击毁率 96%、每架开火 0.00、到达/秒 **0**——敌机朝玩家下来，只要射程不受限追踪一定撞得上，而满配每秒 148 发全命中会把上半屏清空。②**只在近距离锁定**：威胁回来了（射程 110 时到达/秒 15.33 达标），但 110 像素几乎贴着玩家，**玩家根本看不见追踪**——真人试玩直接反馈"没有追踪效果"，等于功能没做出来。量清边界后确认**可见与有威胁直接冲突、调参救不了**（射程 110 → 15.33 达标、150 → 11.67 掉线）。③ 所以改成现在这样：主弹幕照直（弹幕墙与拦截弹不受影响），追踪由**少量、全屏锁定、明显可辨的导弹**提供，用**发射节奏**（`seeker_interval`）控制总输出。它是这台战机的平衡支点：实测间隔 1.0 → 11.50 掉线、1.4 → 13.50 达标、1.6 → 14.33。导弹还刻意**不继承穿透**，否则一发能连杀 4 架（实测把击毁率从 56% 推到 80%）。
- **低速模式与判定范围**：按住 Shift 移速按 `focus_speed_scale`（默认 0.5）减慢，同时显示**真实的碰撞范围**。这里刻意没有做成弹幕游戏常见的"中心一个 2 像素判定点"：本作命中盒是 34×42 的矩形（比机身轮廓小一圈，留了容错），画一个点会让玩家以为自己比实际更难被击中。显示矩形的顶点**直接从 `CollisionShape2D` 的形状推导**，而不是在场景里另抄一份——一个与真实判定不符的提示比没有提示更糟。
- **暂停与设置**：Esc 打开暂停面板，可以开关屏幕震动、分别调整音乐与音效音量、继续或重新开始。三项设置写进 `user://save.cfg` 的 `settings` 节并在下次启动读回；音量改的是**音频总线的实际音量**（0% 时真的静音总线），而不是只存一个数。初始值从总线当前音量反推，所以面板一打开显示的就是实况——总线布局里 Music 是 -6 dB，面板会显示约 50% 而不是写死的 100%。
...
九个独立场景把职责拆开：Main 管规则，Player 管操控，Enemy 管敌人，Boss 是带状态机的强化版敌人，两种 Bullet 管弹药，**Beam 是一条持续生效的致命光柱**，Explosion 只管播一段特效，HUD 管展示。练习场景实例化、节点路径、向量、delta、输入映射、Area2D、Timer、@export、@onready、signal、Tween、await、ConfigFile 存档、ProgressBar、Camera2D、粒子系统、AudioStreamPlayer 与音频总线、自定义 Resource 参数表、多血量敌人的状态机，以及资源文件。进一步学习幂等碰撞、延迟释放、异步跨局隔离、存档容错、暂停语义（含哪些节点必须设 PROCESS_MODE_ALWAYS）、派生状态与手动改写的冲突，以及自动化回归测试。无贴图依赖让问题集中在逻辑而非素材导入——爆炸的粒子贴图用内置 GradientTexture2D 画出，音频由脚本合成，没有引入任何外部素材文件。
...
| Beam（聚焦型光柱） | 全不选 | 第 2、4 层 | 0 | 10（2 + 8） | true / false |
...
**光柱是这张表里唯一 Layer=0 的实体**：它不需要被任何人检测（玩家子弹与敌机都不该“撞到光柱”），所以 `collision_layer=0`；但它自己要**同时**找敌机（第 2 层）与敌弹（第 4 层），所以 Mask 与玩家子弹一样是 10，且 `monitoring` 必须开着。它的伤害结算**不用 area_entered，而是每帧轮询 `get_overlapping_areas()`**——光柱是持续存在的碰撞体，敌机可以一直待在柱内，而 area_entered 只在进入的那一刻触发一次，靠它就只能杀掉“刚进柱”的那一架。这也是为什么 `Monitorable` 关掉无所谓：没有任何东西会来检测它。
...
│   └── HitboxHint (Node2D) [初始 Hidden；按住 Shift 时显示]
│       ├── Area (Polygon2D) [半透明红；顶点在 _ready 里按碰撞形状推导]
│       └── Core (Polygon2D) [中心菱形，给一个视觉锚点]
├── CollisionShape2D (CollisionShape2D) [RectangleShape2D, size=(34,42)]
└── Muzzle (Marker2D) [position=(0,-43)]
```

默认速度 340 px/s，射击冷却 0.16 s，无敌 1.2 s，边缘留白 (30,36)。玩家中心范围 x=30..450，y=118..764，上方预留 HUD。命中盒有意略小于飞机轮廓，提高躲避容错——**这正是要显示判定范围的原因**：轮廓 54×60、命中盒 34×42，玩家看不到自己其实有那一圈余量。按住 Shift 时移速按 `focus_speed_scale`（默认 0.5）减慢并显示该范围；顶点由 `_sync_hitbox_hint()` 从 `CollisionShape2D` 的 `size` 现算，所以改了形状提示会自动跟着变，不可能出现"提示与判定不一致"。

**触屏操作（Web / 手机版的入口）**：手指按住时飞机跟到指尖上方 64 像素（`touch_lift`，少了手指会盖住飞机和它正前方那条弹道），并**自动持续射击**——手机上再单开一个开火键只会占掉本来就小的屏幕。三条实现约束都是踩出来的：

- **只认 `InputEventScreenTouch` / `InputEventScreenDrag`**。Godot 默认把触摸**模拟成鼠标**事件（`emulate_mouse_from_touch`），反过来不会，所以桌面端用鼠标点界面绝不会误拖飞机。这条前提写在这里：哪天有人把 `emulate_touch_from_mouse` 打开，桌面端就会开始"点一下屏幕飞机就飞过去"。
- **跟手是"指数逼近"，不是瞬移**：跟手速度 = `max(移速, 距离 × touch_follow_scale)`（默认 14）。瞬移会让"引擎强化"这条能力在手机上彻底失效；而指数逼近的代价是**稳态滞后 = 手指速度 ÷ 倍率**——手指匀速划 500 像素/秒时飞机落后约 36 像素。**这个倍率就是"跟不跟得上"的全部秘密。**
- **抬手立刻停火并清状态**，`reset_for_game()` 与 `deactivate()` 都会清掉 `touching`：否则上一局残留的状态会变成"没碰屏幕却在开火"。

跟手的验证方式也是这轮踩出来的：无头模式下窗口是 **0×0**，引擎的屏幕变换会退化成 8%，注入的触点坐标被换算到一万像素以外，飞机直接被夹到右下角。所以回归测试把两件事**分开验**——"事件坐标换算对不对"用引擎自己的 `get_screen_transform()` 反算期望值，"跟手逻辑对不对"直接给一个战斗区内的游戏内坐标。
...
├── Visual (Node2D)
│   ├── Shield (Polygon2D) [拦截光环，初始 Hidden；只有 intercepts 为真时才显示]
│   ├── Body (Polygon2D) [弹体：形状与颜色**按当前战机**切换]
│   └── Core (Polygon2D) [亮芯：按弹体轮廓等比缩小现算，不在场景里另抄一份顶点]
└── CollisionShape2D (CollisionShape2D) [RectangleShape2D, size=(8,20)，不随战机变化]
```

**每台战机只用自己的弹体。** `body_polygon` / `body_color` / `core_color` 由 Main 在 `add_child` 之前从 `SHIPS` 表里取好（表是唯一来源）：标准型是短弩形青蓝弹，游隼型是细长镖形青绿弹（与它的机身、座舱同一族颜色），追踪导弹则是另一套带尾翼的弹形（轮廓在 `Main.SEEKER_HULL`，因为它属于那条武器而不是"主弹幕的形状"）。

**视觉可以有个性，判定盒不能有个性**：弹形与配色随战机变，`CollisionShape2D` 始终是 8×20。这条是刻意钉死的——命中盒一旦跟着外观走，"看得见"和"打得中"就变成两件事，而这正是本项目反复吃亏的类型。回归断言同时验"外观确实不同"与"判定盒完全一致"。

**拦截弹的专属外观**：`intercepts` 为真时每颗子弹会显示一圈 `Shield` 光环。它存在的理由是"看不看得见"——拦截弹以前只是一个开关，玩家拿到之后画面上毫无变化；现在"我的弹幕能挡子弹了"这件事直接长在子弹上（`VisualTest` 里有一张专门的截图）。
...
`homing` 与 `homing_turn_rate` **只对追踪导弹生效**（游隼型），主弹幕一律照直飞。转向时子弹会同时旋转图形，玩家因此看得出"这发在拐弯"；平行弹幕与主弹幕的 `direction` 恒为正上方、旋转量恒为 0，与旧行为逐值一致。**目标来自一个共享的组查询**（`get_first_node_in_group("player_target")`），而不是每颗子弹各自遍历敌机——满配每秒 148 发，逐弹遍历会变成每秒几千次全表搜索。这个写法与 `Enemy.gd` 瞄准玩家时用 `get_first_node_in_group("player")` 是同一个套路。

**"有没有锁定"用指示器在不在组里表示**，而不是把指示器摆到一个替身坐标上。第一版就是在没有目标时把它放到玩家正上方，结果两侧车道的子弹全都朝中间拐、弹幕从"一排平行车道"变成"一束"——症状很好认：锁定射程怎么改都量出逐位相同的结果。现在没有锁定就把指示器摘出组，子弹照直飞。
...
├── MessagePanel (Panel) [30,182 → 450,558]
├── ShipCaption (Label) [45,192 → 435,214; 居中; font_size=14]
├── ShipButton0 (Button) [45,218 → 169,272; 选中态深青底白字]
├── ShipButton1 (Button) [177,218 → 301,272; 同上]
├── ShipButton2 (Button) [309,218 → 433,272; 第三台战机；三台时由 show_ship_choice() 按可用宽度均分]
├── ShipDetail (Label) [45,278 → 435,300; 居中; font_size=13]
├── MessageLabel (Label) [45,306 → 435,436; 水平/垂直居中]
├── StartButton (Button) [82,452 → 398,500; Focus=None]
├── RestartButton (Button) [同上; 初始 Hidden; Focus=None]
├── ChangeShipButton (Button) [82,508 → 398,550; 初始 Hidden; 只有结算页显示]
...
├── PauseOverlay (ColorRect) [0,90 → 480,756; 深色半透明; 初始 Hidden]
├── PausePanel (Panel) [56,236 → 424,566; 初始 Hidden]
├── PauseTitle (Label) [70,252 → 410,288; font_size=22; 居中]
├── ShakeCaption (Label) [76,306 → 228,334; font_size=16]
├── ShakeButton (Button) [240,300 → 404,340; Focus=None; 文字是 开/关]
├── MusicCaption (Label) [76,356 → 228,384; 同上]
├── MusicSlider (HSlider) [240,356 → 404,384; 0~100, step=5]
├── SfxCaption (Label) [76,400 → 228,428; 同上]
├── SfxSlider (HSlider) [240,400 → 404,428; 同上]
├── PauseResumeButton (Button) [76,452 → 404,496; Focus=None]
└── PauseRestartButton (Button) [76,508 → 404,552; Focus=None]
```
...
TopBar、LevelLabel、BestLabel 与 XpBar 都在 y=90 以上，而 Overlay 从 y=90 起才覆盖，因此等级、经验与最高分在开始、战斗、升级、结算四种状态下都可见，弹窗里无需重复显示——这也是它们放在顶栏而不是消息面板的原因。**本局战报正是按这个前提写的**：它刻意不重复分数、最高分、等级与剩余生命，只补顶栏看不到的击毁数、最高倍率、受伤次数、漏敌架数与能力路线，一共 6 行。MessageLabel 高 130px、字号 18，6 行约 140px——**这里已经贴着极限**，改文案前先看回归断言（它对宽度和高度两项都做了判断）。

**开始界面的战机选择**：三个按钮的位置**按实际台数现算**（`show_ship_choice()` 里按可用宽度均分），不写死三档坐标——否则加第四台时新按钮会叠在旧的上；因此下面节点树里写的是初始矩形，运行时会以均分后的位置为准。选中态需要**明显可辨**：主题的 pressed 与 normal 太接近，所以给按钮单独挂了深青底白字的 `pressed` / `hover_pressed` 样式；第一版只靠 `ButtonGroup` 的默认按下态，截图里不同按钮看起来一模一样。

**暂停面板为什么由 HUD 来处理按键**：Esc 必须**在暂停中也能收到**。Main 是默认的 PAUSABLE 节点，整树一暂停它的 `_unhandled_input` 就不再执行——那样玩家会被永久卡在暂停里、再也按不回来。所以暂停键交给 HUD（根节点 `process_mode = 3`），HUD 只发出"玩家按了 Esc"这一个信号，**改不改规则仍由 Main 决定**（`Main.toggle_pause()` 只在状态为 PLAYING 时才接受）。同一个理由也适用于面板上的按钮：整树暂停时信号回调照常执行，所以滑块与开关依然有效。

暂停面板的控件全部落在面板矩形内，这一点有回归断言；按钮的即时反馈由 HUD 自己维护（按下震动开关会立刻改文字），而不是等 Main 回过来刷新——第一版就是等回调，结果按下之后文字不动，被断言抓到了。

**升级面板与暂停**：HUD 根节点必须设 `process_mode = 3`（PROCESS_MODE_ALWAYS）。升级时 Main 执行 `get_tree().paused = true` 冻结整个世界，而 Godot 不会把输入派发给 PAUSABLE 节点，若 HUD 仍是默认的 INHERIT，五张能力卡会看得见却点不动、数字键也失效。把这一个根节点设为 ALWAYS 后，其子节点默认 INHERIT 一并生效。这**不是**全项目唯一需要碰 process_mode 的地方——音乐、音效池与爆炸也都要设 ALWAYS（见 5.1），暂停面板上的按钮同样依赖它；但它是**第一个**必须先设对的地方，否则连暂停菜单都点不动。UpgradeCard 的 `focus_mode` 为 None，与开始/重开按钮一致，避免空格变成激活卡片的按键。
...
```text
Beam (Area2D) [Beam.gd; Layer=0, Mask=10; Monitorable 关掉无妨]
├── Visual (Node2D)
│   ├── Glow (Polygon2D) [2 倍宽的半透明光晕，纯装饰]
│   └── Core (Polygon2D) [亮芯：宽度 = 判定宽度]
└── CollisionShape2D (CollisionShape2D) [RectangleShape2D, size=(10,200)；运行时按真实长度重建]
```

**它与其余实体的本质区别是：它持续存在，而且几何每帧都在变。** 子弹是“生成 → 飞 → 命中或出界 → 自己释放”，光柱则**在一串之内**一直挂在屏幕上，长度随玩家上下移动而伸缩，条数与横向位置则**照抄子弹车道的算法**——`_lane_offsets()` 是子弹与光柱共用的唯一来源，所以“火力增援/侧翼炮”在这台战机上含义完全相同，两台战机的差别只剩“瞬间命中 vs 飞行时间”和“覆盖窄 + 射程短 vs 弹幕墙”。

**光柱是"一串一串"的，不是常驻光环**：按下开火开始一串（`beam_burst_seconds` 0.45 秒），一串放完等 `beam_burst_gap_seconds`（0.14 秒）才允许下一串，按住开火就是一串接一串。两个后果必须一起处理：

- **占空比 = 时长 ÷（时长 + 间隔）**，它**必须**进 `player_dps_proxy()`。光柱的每秒伤害是"车道数 ÷ 结算间隔"，而断续之后实际输出只有这个值的七八成；不折算的话 Boss 会按"一直亮着"的血量出场，Boss 战被硬生生拖长。折算了以后，把光柱改成断续只是把**输出和血量一起按比例缩小**。
- **射速强化缩短的是串间间隔**（不是结算间隔——那是弹速强化的事），因此它在这台战机上真实有效，而不是死选项。

**长度 = min(射程, 到战斗区顶边的距离)**，两条边界各管一件事，都由 `Main.beam_length()` 算：

- **射程**（`beam_range`，默认 340）：这是这台战机的第二项代价，也是真人反馈“太赖皮”之后加的。没有它，玩家可以永远待在屏幕底部把上半屏扫干净，还能**站在安全距离把 Boss 融化**——站桩实测 141.8 发/秒的输出，而子弹系在同一个位置只有约 72/秒。加上之后这个数字变成 0，想打 Boss 必须主动上前。**这个数不是随手取的**：340 时玩家停在下方、光柱上端离 Boss 下沿还有约 47 像素，那条捷径才算真正被切断；放宽到 380 时只要往上挪十来个像素就能碰到 Boss，限制就退化成装饰了。
- **战斗区顶边**（`play_area_top`，默认 90）：光柱不会伸到顶部信息栏后面去杀还没露头的敌机，理由与玩家子弹出屏即失效完全一致（见第 1.1 节）。

三个实现细节值得说明：

- **尺寸只在真的变了才重建**：`refresh()` 先比较宽与长，相等就直接返回。每帧重建多边形与碰撞形状会白白制造对象，改动 `RectangleShape2D.size` 还会让物理服务器每帧重算一次，没有意义。
- **亮芯宽度 = 判定宽度**：多边形顶点由 `beam_width` 现算，`Glow` 是 2 倍宽的装饰，`Core` 就是 `beam_width` 本身。这与 Player 的判定提示是同一条纪律——看得见的“实体”必须等于真正会造成伤害的范围。回归断言把“亮芯宽度 = 碰撞形状宽度 = `beam_width`”三者钉在一起。
- **每帧同步的不只是几何，还有结算间隔与拦截开关**：玩家是在**局中**拿到“弹速强化 / 拦截弹”的，而光柱开局就在屏幕上。第一版只在创建时写了这两个值一次，于是升级之后屏幕上的光柱仍按旧节奏结算、也仍然不拦敌弹——**玩家会认为升级没生效**，而所有断言照样是绿的（它们断言的是 `beam_tick_interval()` 这个函数的返回值，不是屏幕上那根光柱）。现在两者都走同一次 `refresh()`，并各有一条针对“已经存在的光柱”的断言。**这是本项目第三次踩到同一类坑：值只在“创建时”写对了，之后它就不再跟着规则走。**
### 5.10 波次与难度参数表
...
下面十个文件是项目实际使用的完整源码，不是伪代码。所有信号通过代码连接，场景无需额外编辑器信号连接。各脚本不注册 class_name，避免导入时的全局类扫描依赖；场景实例经鸭子类型调用已约定的方法，基础数值和 Godot 节点仍尽量显式类型。

职责链：Player 请求开火 → Main 延迟实例化 PlayerBullet → PlayerBullet 调 Enemy.take_hit → Enemy.destroyed(points) → Main.add_score。**聚焦型没有子弹，这条链断在第一步**：`_on_player_shoot_requested()` 直接返回，伤害改由光柱每帧轮询结算（`Beam._strike()` 自己调 `Enemy.take_hit()`），但“谁算分”没有变——仍然是 `Enemy.destroyed`，所以两台战机的计分、连击、经验走的是同一条路。玩家受伤由 Player.player_hit → Main 统一扣命；HUD 只展示。
...
**光柱也没有新增任何信号**：它既不发信号也不连信号，靠 `get_overlapping_areas()` 轮询，命中时直接调对方的 `take_hit()` / `intercept()`——与 PlayerBullet 用的是同一对方法，所以敌机与敌弹那一侧完全不知道伤害来自子弹还是光柱。代价是它必须自己管“多久结算一次”（`tick_interval`），而这个节奏同时就是它的每秒伤害，会进 `Main.player_dps_proxy()` 参与 Boss 血量折算。
...
3. 将第 6 节十个脚本分别保存到对应 scripts/*.gd，将第 5.12 节九个场景与两份 .tres（主题与波次参数）保存到对应路径，编码 UTF-8。
...
1. **Player**：新建场景 → Other Node → Area2D，重命名 Player；按树加 Visual Node2D 和三个 Polygon2D。选 Polygon2D 在 Polygon 属性输入第 5.12 节的顶点坐标，或直接拖入交付场景。加 CollisionShape2D → New RectangleShape2D → Size 34×42；加 Marker2D 改名 Muzzle、Position(0,-43)。Layer 仅第1层，Mask 仅第2、4层，挂 Player.gd，保存 scenes/Player.tscn。
...
7. **波次参数表**：新建资源 —— 在 FileSystem 里右键 assets → New Resource → 搜 `WaveTuning`（即 `res://scripts/WaveTuning.gd`）→ Create，命名 `wave_tuning.tres`。Inspector 里会看到四大组参数；保持默认值即可，也可以按第 5.10 节的说明自行调整。
...
9. **Explosion**：新建场景 → Other Node → CPUParticles2D，重命名 Explosion；关掉 Emitting、勾上 One Shot，Amount=20、Lifetime=0.42、Explosiveness=1、Spread=180、初始速度 90..240、重力 (0,120)。Texture 选 New GradientTexture2D，宽度高度都设 24、Fill 选 Radial、gradient 设为“亮黄 → 橙 → 全透明”。再加一个 Polygon2D 子节点改名 Flash，用第 5.12 节给的八角形顶点、颜色纯白。挂 Explosion.gd，保存 scenes/Explosion.tscn。
10. **Boss**：新建场景 → Other Node → Area2D，重命名 Boss。Layer 第 2 层、Mask 全关、Monitoring 关闭、Monitorable 开启（与 Enemy 完全一致，因为两者复用同一条检测链路）。加 Visual Node2D 与 Hull/Plate/Core 三个 Polygon2D（顶点见第 5.12 节）、CollisionShape2D（矩形 150×78）、Marker2D 改名 Muzzle、Position(0,52)，以及 ShootTimer（Wait Time 1.5、One Shot 勾选、Autostart 不勾选）。挂 Boss.gd，保存 scenes/Boss.tscn。
11. **Beam**：新建场景 → Other Node → Area2D，重命名 Beam。**Layer 全不选（`collision_layer=0`）、Mask 勾第 2 与第 4 层**、Monitoring 开启（它要自己轮询敌机与敌弹），Monitorable 随意。加 Visual Node2D，其下加两个 Polygon2D：Glow（半透明浅蓝）与 Core（亮芯），再加 CollisionShape2D（矩形 10×200）。**顶点与形状不用手填精确值**——它们每帧由 `Beam.refresh()` 按 `beam_width` 与真实长度重建，第 5.12 节里的数值只是编辑器里的初始预览。挂 Beam.gd，保存 scenes/Beam.tscn。
...
- **判定范围应当与真实命中盒一致**：按住 Shift 时出现一个半透明红色矩形，它必须比机身轮廓**小一圈**（轮廓 54×60、命中盒 34×42）。如果它看起来和机身一样大，说明提示写错了。
- **暂停面板**：Esc 能打开与关闭；面板里改音量时应当**立刻听得出变化**，拖到 0 应当完全静音；点“重新开始”后分数与生命都复位，且不会停在暂停状态。
...
项目额外附带 `res://tests/SmokeTest.gd`，是纯 GDScript 的 SceneTree 回归测试，不属于正式游戏节点树，不参与F5。包含按钮、输入、移动边界、真实物理碰撞、重复计分、无敌、清场、难度、射击敌人、越界、跨局计时、最高分持久化（含无存档、低分不覆盖、新实例读回、存档损坏回退）、本局战报与对局记录（含击毁数只认真击毁、漏敌不计入、峰值倍率不因受伤清零而回退、结算会清空连击并收起顶栏倍率、记录字段逐项与本局核对、只保留最近 50 局且裁掉的是最旧的、记录文件被写坏后仍能正常追加且坏行被丢弃、战报宽高都不溢出、记录带可随参数变化的规则指纹、受伤时间点序列单调不减且能识别出“连续送死”的对局），等级与经验、升级暂停、16 项能力逐项生效、多发弹生成与竖直平行弹道、侧翼炮对称性、弹体增幅真实缩放、拦截弹（含未开启时的对照组）、紧急清屏只清敌弹、得分与经验倍率互不干扰、穿透弹与对照组、幸运补给把选项加到 5 个、连续升级、死选项过滤、重开复位、爆炸生成位置与自行释放（含“击杀当帧触发升级时爆炸不被丢掉”这条回归）、受伤震动（含关闭选项与结算归零）、受伤红闪与得分脉冲的时间行为、音频总线与循环点、音效池配置与五类事件各自的触发、波次编队与出场位置、波次推进与波间停顿、速度/弹速/射击间隔/开火概率/瞄准比例的上下限夹紧、连击倍率的阶梯与上限、受伤与漏敌都会打断连击、漏敌累计并抬高难度压力且压力封顶、有效难度上限随玩家等级放宽、难度撞到下限之后仍持续收紧（含“难度 1~12 与线性公式逐值一致”的零回归对照，以及难度 20/30/40 逐档更密的平台期守卫）、逃敌压力与时间在同一条轴上合并取整（分数压力必须能跨级，防“各自取整”的旧台阶回归）、敌机齐射（发数阶梯与封顶、一次真的打出多发且都变成场上敌弹、左右严格对称、单发时与旧行为逐值一致的对照）、玩家子弹出屏即失效（成对验证“顶部栏之下的敌机打不到、露头之后打得死”）、拦截弹一发换一发（击落敌弹的同时自身被消耗）、低速模式与判定范围（Shift 绑定与引擎常量核对、判定提示尺寸必须与碰撞形状完全一致、移速按倍率减慢且仍在移动）、**触屏操作**（按下进入触屏态、触点按引擎的屏幕变换换算成游戏内坐标并抬高 `touch_lift`、按住自动持续射击、抬手立刻停火、拖动更新目标、手指滑出边界时飞机贴边而不是被带出去、触屏不打开键盘那套判定提示、操作提示跟着输入方式切换而不叫触屏玩家去按 WASD）、暂停与设置（暂停确实冻结世界且再按能恢复、非游戏状态拒绝暂停、面板控件都在面板内、震动开关的显示与状态同步、音量滑条真的改到总线且 0% 会静音、设置换新实例能读回、从暂停面板重开会解除暂停并复位）、开局选战机（表结构完整且 id 不重复、三台战机的按钮数量与选中态和实际数据一致、按钮按台数动态排开且都在面板内不重叠、切换会换掉外形、配色与说明文字、未知 id 与越界下标都被拒绝、选择能落盘读回）、追踪弹（**锁定射程外的敌机不锁定、进射程就锁定**、按下开火才发射且受发射冷却限制、冷却没走完不会多放、导弹真的会拐弯、比主弹幕更大更亮、**导弹战机不出普通子弹**）、聚焦型的光束（条数与车道数一致、按车道偏移排开并跟随玩家、**玩家在下方时光柱长度等于射程、贴近顶部时改由战斗区顶边截断**、亮芯宽度 = 碰撞形状宽度 = `beam_width`、柱内敌机被瞬间击毁、柱外敌机完全不受影响、**屏幕顶部的敌机在下方够不着、玩家升上去就烧得到**、柱内敌弹被击落、光束战机不生成子弹、弹速强化缩短结算间隔、局中拿到弹速强化/拦截弹后**已经存在的光柱**立刻换节奏并开始拦敌弹、穿透弹对光束不提供而标准型仍然提供、换回标准型后光柱消失）、**每台战机只用自己的弹体**（两台出子弹的战机的弹形与配色互不相同、追踪导弹是带尾翼的另一种弹形且为暖色、**弹形不同但判定盒完全一致**、拦截弹的光环只在开启后出现且不会改变判定盒）、爆炸粒子（数量与尺寸下限、**生成位置取"粒子被发射那一刻"而不是节点事后被挪到的位置**、播完自行释放、跨局与状态的守卫）、视觉收尾的回归守卫（4 张与 5 张时升级面板都必须紧贴最后一张卡、Boss 顶部轮廓至少有三种不同高度、Boss 机体顶边与血条之间留有空隙、受伤红闪峰值不超过 0.30、爆炸粒数与尺寸的下限、背景是竖向渐变贴图且航道线不少于 5 条），瞄准型敌机的弹道确实指向玩家且换用不同颜色标注、波次倍率真实生效、每轮数量加成及其上限、参数表缺失时的兜底、Boss 的登场时机与预警、入场与巡航边界、扇形齐射的方向与真实生成、撞机不自我消耗、多血量挨打与击破结算、Boss 血量随玩家输出变化（对比两套差距很大的 build）、血条同步、以及重开清场，还有固定矩形 UI 的溢出与重叠断言。它会在开头清空 user:// 存档以保证可重复，因此请只对测试用的 Godot 用户目录运行。不要把它挂到 Main。另附 VisualTest.gd 在真实图形模式下生成开始/战斗/暂停/特效/拦截弹光环/追踪导弹/贯穿光束/Boss/升级/结算共十张截图，它会短暂显示游戏窗口，属于开发验收脚本，不属于正式玩法。SmokeTest / VisualTest / StressTest 三个脚本的完整代码列在第 6 节末尾，另外三个基准（ThreatTest / SoakTest / PursuitTest）随交付 ZIP 一起提供，其门槛与设计理由见本节上文。

**`res://tests/StressTest.gd` 是压力基准，不是玩法测试**。它把所有攻击类能力叠满、按住射击打满 6 秒，量出峰值对象数、真实帧时间与清场后的残留，用来支撑“要不要做对象池”这个决策（结论见下一条）。**它显式钉住标准型**：战机是写进存档的持久设置，而威胁基准会把三台都跑一遍、最后停在聚焦型；不钉住的话这条基准量的就是“上一条测试留下的那一台”，流程顺序会悄悄改变结论（守卫断言已于事后补上）。它用**墙钟**测帧时间而不是 `Performance.TIME_PROCESS`——无头模式下后者报出来的值明显偏大（同一段实测 33 ms，但 720 步根本不可能跑满 24 秒），拿它当帧时间会得出与事实相反的结论。敌机按**游戏真实的刷怪间隔**补（直接取 `get_spawn_interval()`），而不是写死一个步数——写死 0.3 秒一架比真实高难度还稀疏，“峰值在预算内”对敌机密度就不成立了。帧时间报 **p99** 而不是最大值：单次最大值被调度噪声主导（同一份代码四次运行量到 14.21 / 17.13 / 14.21 / 78.85 ms，而平均帧稳定在 8.24 ms）。

**`res://tests/SoakTest.gd` 是长时浸泡基准，回答“长期局会不会越打越卡”**。压力基准只测 6 秒，能证明“瞬时负载在预算内”，证明不了“时间长了会不会变差”。它让满配玩家连续打满 60 秒（压力基准的十倍），对比**前后半段的帧时间**与**开头/结尾的实体峰值**，并断言停火后残留为 0、对象净增有上限。实测：前半 8.33 ms → 后半 8.33 ms（毫无漂移）、实体峰值 112 → 105、残留 0、对象净增 9。**它的边界也写清楚**：60 秒仍不是完整的一局（真人一局 2~5 分钟），这条基准没有覆盖到那么长。它同样固定了随机种子，否则前后半段的对比会被出生位置的随机差异淹没，也**同样显式钉住标准型**——这里踩的坑更典型：战机的选择是持久设置，前一条基准（威胁基准）跑完会停在聚焦型，于是这条基准第一次跑出来是 `残留Actors=11`（11 正好等于满配的光柱条数）：光柱是**持续存在的武器本身**，只要 state 还在 PLAYING，`_sync_beams()` 下一帧就会把它重新建出来，“停火后残留必须为 0”这条断言对光束系根本不成立。两条基准现在各带一条“量的是子弹系满配”的守卫断言，防止再次悄悄量错对象。

**`res://tests/ThreatTest.gd` 是威胁基准，量“后期还有没有威胁”**。它要回答的问题很具体：**敌方的火力有多少真的能穿过玩家弹幕、到达玩家面前**。指标有两个——**敌弹通过率**与**每架敌机开了几枪**——后者是核心诊断量。玩家固定在屏幕下方且全程不动，所以通过率与躲闪技术无关；同时它也是“最差情况”的命中数。这条基准的由来是一次方向性错误：真人反馈“一点威胁都没有”，而我此前几轮一直在**敌人那一侧**加码（密度、速度、开火概率、瞄准比例、难度曲线），而基准量出的真相是**每架敌机平均只开 0.69 枪**（按存活时间本该 2~4 枪）——玩家每秒 148 发、11 条弹道，把敌机在开火窗口之前就打死了，那些参数乘的都是“敌机能开几枪”，而它已经被压到接近 0。修法是齐射（见第 1.1 节）。它同样把结论固化成了断言：最高难度下每架敌机 ≥ 2.5 发、每秒 ≥ 12 发到达（修复前是 0.69 发与 3.33 发），一旦威胁再次被抹平就会失败。**门槛之所以敢贴着实测值定，是因为基准固定了随机种子**：出生横坐标、开火与瞄准判定都走 Main 的 rng，不固定时“到达/秒”会在 8~19 之间跳，下限定到 8.0 还是会偶发失败；固定后同一份代码连跑三次是 19.83 / 20.33 / 19.83。**注意：它必须把 `xp_multiplier` 设为 0**，否则击毁敌机会触发升级抉择把整树暂停，`_create_enemy` 直接返回，量出来的“通过率 0%”是一个由故障伪装成的结论（这个坑第一版就踩了）。**它还遍历全部三台战机**：形态把平衡面翻了一倍多，靠手调守不住，所以“每台都必须达标”被写成了构建级断言——最高难度下实测标准型 20.33、聚焦型 17.17、游隼型 13.67 发/秒到达。游隼型第一次跑出来是 `每架开火 0.00 / 到达每秒 0.00`，正是被这条断言当场拦下的。顺带记一个反直觉但可解释的结论：**越“能杀”的武器，敌方火力反而越少**——光束出膛即中，把敌机存活时间压到最短，所以它的到达量低于标准型；这也说明“形态只改武器行为”并不意味着平衡工作量减半，反而必须逐台量过。

**`res://tests/PursuitTest.gd` 是追击基准，量"玩家按人的方式打"时三台战机差多少**。威胁基准把玩家钉死在屏幕下方并且盲目扫射，那套走法对**瞬间命中**的光束是失真的：真人会看着敌机打、会往够得着的地方挪。这条基准里的玩家只朝最近的敌机靠过去（横向 + 纵向，每 0.15 秒更新一次目标，模拟反应延迟）、**完全不闪避**，所以它量的是"武器本身能打成什么样"，不是"人能不能活下来"。**它是被真人反馈"聚焦型太赖皮"逼出来的，一上来就量到两个威胁基准完全看不见的东西**：① Boss 血量按"每秒发数 × 7 秒"反推，公式默认发发命中——子弹实际只兑现约 74%（Boss 会巡航、子弹要飞），光柱却接近 100%，于是公式给出的 1925 点被 1400 的血量上限砍到 1400，Boss 5.1 秒就被打空，而子弹系要 9~10 秒；② 光柱原本直达战斗区顶边，**站在屏幕底部不动**就能对 Boss 打出 **141.8 发/秒**（子弹系在同一个位置只有约 72/秒）。这两条合起来就是"赖皮"的量化定义，修法见第 5.9 节。**它还纠正了我自己的一次误判**：同一套配置在 6 秒窗口里量出三台击毁率 62% / 80% / 84%、差距 22 个点，看上去"削过头了"；把窗口放长到 10 秒才发现那是窗口长度造成的假象——敌人得先飞进光柱射程，短窗口把"需要敌人靠近"的武器的前半段算成了"没打中"。10 秒下实测 80% / 87% / 84%，前场基本持平。**"量到了"和"量准了"是两件事。**

**无头也能验音频**：`--headless` 用的是 Dummy 音频驱动，听不到声音，但 `AudioStreamPlayer.playing` 反映的是真实的播放状态，所以“射击是否触发了射击音效”这类断言在无头下依然成立，不需要真的发声设备。

**每个测试脚本都带看门狗**。GDScript 没有 try/catch：`_run()` 里任何一处运行时错误（例如访问一个已经不存在的属性）都会让协程当场中断，末尾的 `quit()` 永远执行不到，进程就一直挂着——外层只能等到超时，报错还是一条看不出原因的 `TimeoutExpired`。看门狗把“挂死”换成一条明确的错误信息和非零退出码。这个问题在本项目里真实发生过一次，因此固化成了机制。
...
8. **重新发布后怎么让访问者看到新版**：`export_web.py` 会把主包改名成带内容指纹的 `plane-<hash>.pck`，并写进 `index.html` 的 `mainPack`（这是网页外壳官方支持的"另一个 pck 名"：`index.js` 里就是 `const pack = this.config.mainPack || `${exe}.pck``）。不做这一步的话，Pages 的 `max-age=600` 会让浏览器拿到新 html 却继续用十分钟前缓存的旧 pck——**"我明明改了、打开还是旧的"**就是这么来的，真机上已经发生过一次。`index.html` 自身仍会被缓存，等一会儿或强刷（Ctrl+F5）即可，想立刻绕开就在链接后加查询串。
...
以下是可选扩展，不是运行前置条件；本版已经实现难度递增与波次编排、Boss、无敌闪烁、最高分保存、16 项能力的等级抉择系统、三种开局战机形态（标准型 / 游隼型 / 聚焦型）、爆炸粒子、受伤屏幕震动与视觉反馈（命中白闪、受伤红闪、得分脉冲），以及音效与循环背景音乐，其余默认未实现，不依赖任何缺失素材。
...
上表中已经落地的几项不再重复列出，实现位置如下：最高分保存见第 6 节 `Main.gd` 的 `_load_best_score()` / `_save_best_score()`；本局战报与对局记录见 `Main._build_run_report()` / `build_summary()` / `_append_run_log()` 与 `HUD.show_game_over()`；三种战机形态见第 1.1 节与 `Main.SHIPS`；聚焦型的光柱见第 5.9 节；爆炸粒子见第 5.6 节的 Explosion 场景；受伤屏幕震动见第 5.1 节的 Camera2D 与 `Main._start_screen_shake()`；命中白闪、受伤红闪与得分脉冲见第 5.6 节与 5.8 节；射击/命中音效与背景音乐见第 5.1 节的 Music / Sfx 节点说明与 `Main._build_audio()`；波次与难度参数表见第 5.10 节；Boss 见第 5.7 节。
...
**补充证据（长时浸泡基准）**：压力基准只测 6 秒，“长期局会不会变差”它答不了。外部评估据此提过“满配每秒 instantiate/free 上百节点，长期局有 GC 抖动风险”。`tests/SoakTest.gd` 让满配连续打满 **60 秒**（十倍），把这个问题变成了可测量的：**前半段 8.33 ms → 后半段 8.33 ms，实体峰值 111 → 105，停火后残留 0，对象净增 9**。也就是说对象周转在这段时间里**完全没有随时间退化**——不是“看起来还行”，而是前后两半的量值相同。这条担忧到此可以结案；若将来出现漂移，SoakTest 会先失败。
...
| 按了暂停却再也退不出来 | 暂停键交给 PAUSABLE 的 Main 处理；整树一暂停它的输入回调就不再执行 | 暂停键由 HUD（PROCESS_MODE_ALWAYS）处理，只发信号，由 `Main.toggle_pause()` 决定是否接受 |
| 暂停面板里点“重新开始”没反应 | 此时 state 仍是 PLAYING，而 `start_game()` 默认拒绝这种重开（防误触 R） | `start_game(force)` 加一个显式参数，只有暂停面板走这条路径 |
| 按下开关按钮后文字不变 | 按钮文字只在 `show_pause()` 里刷新，要等 Main 回过来才更新 | 按钮自己维护显示状态并即时切换，再发出期望值；否则玩家会以为没生效 |
| 面板显示的音量与实际不符 | 初始值写成 100%，而总线布局里 Music 是 -6 dB | 初始值由 `db_to_linear(总线音量)` 反推；0% 直接 `set_bus_mute`，而不是设一个很小的音量 |
| 判定提示与真实判定不一致 | 在场景里另抄一份尺寸，改碰撞形状时忘了同步 | 顶点在 `_ready` 里由碰撞形状现算；回归断言两者必须相等 |
| 逃敌压力让难度“突然”跳一级 | `int(时间)` 与 `int(压力)` 各自取整后相加，压力每跨过 1.0 就凭空多一级 | 压力累加到时间轴上再取整；断言分数压力（0.6 级时间 + 0.5 级压力）必须能跨级 |
| 追踪弹把威胁整个抹平 | 敌机朝玩家下来，射程不受限时追踪必然命中；弹群密度又高（满配在飞约 170 发） | ①先把追踪限制在近距离（保住威胁但玩家看不见）→ ②改为**主弹幕照直 + 少量全屏锁定的追踪导弹**，用发射节奏控平衡（间隔 1.4 → 到达/秒 13.50 达标） |
| 追踪导弹一发能连杀多架 | 它继承了玩家的穿透层数（穿透×3 时一发能杀 4 架），实测把击毁率从 56% 推到 80% | 导弹设为**单发目标**（`pierce_left = 0`），"发射节奏 → 击杀数"变成一比一，平衡才好预测 |
| 追踪弹的射程参数怎么改都量出同一结果 | 没有锁定时把指示器摆到玩家正上方当替身，两侧子弹全朝中间拐、弹幕变成一束 | 用“指示器在不在组里”表示有没有锁定；没锁定就摘出组，子弹照直飞 |
| 换了战机但开始界面的说明文字不变 | 点击按钮只改了状态，`ship_detail` 只在 `show_start()` 里赋过值 | `set_ship()` 之后让 HUD 刷新；刷新只在开始界面可见时进行，免得战斗中凭空冒出按钮 |
| 光束战机上“弹速强化 / 拦截弹”看起来没生效 | 光柱是开局就在屏幕上的**持续实体**，而这两个值只在**创建时**写了一次；升级发生在光柱已经存在之后 | 结算间隔与拦截开关改走每帧的 `Beam.refresh()`；断言必须针对**已经存在的那根光柱**，而不是只断言 `beam_tick_interval()` 这个函数的返回值——后者一直是绿的 |
| 光柱看起来比实际判定更宽（或更窄） | 多边形顶点与碰撞形状各写一套比例，对不上 | 亮芯宽度 = 碰撞形状宽度 = `beam_width`，三者由同一组顶点现算；回归断言把它们钉在一起 |
| 光柱"一直亮着"，不像在开火 | 它是开机就常驻的光环，不需要玩家按开火 | 改成按下开火打出一串（时长 + 串间间隔），**占空比进每秒伤害公式**，射速强化缩短串间间隔；断言先守"没按开火时没有光柱" |
| 导弹战机前场打不够（击毁率 60% 对 80%） | 导弹是必中武器，而它的产量被发射冷却卡住；同时"永远朝正上方出膛"让玩家往上压时导弹要先掉头、甚至飞出战斗区 | ①出膛就朝目标方向；②**每枚导弹锁自己的目标**（而不是所有导弹追同一个"最近的敌机"，否则第一枚得手后其余全扑空）；③射程与射速一起调 |
| 给导弹开拦截弹之后它反而打不到敌机（击毁率 73% → 15%） | 导弹全屏锁定、必然穿越整片敌弹空域，开了拦截就会在半路被敌弹消耗掉——它成了"打敌弹的磁铁" | 导弹不继承拦截弹，并且这台战机**不提供**拦截弹：拦截弹的语义是"我的弹幕墙能挡子弹"，那是直弹幕的能力 |
| 必中武器怎么调都是"要么太弱、要么清屏" | 只用了**射速**这一个旋钮：射速高 → 前场达标但静态威胁基准崩（D30 到达 3.5 发/秒）；射速低 → 威胁回来了但前场打不够 | 补上**射程**这个杠杆（锁定射程 420）：射速决定"打得多快"，射程决定"打得到多远"，两者一起才存在同时达标的区间。**一个旋钮调不出两个目标** |
| 光束战机站在屏幕底部就能把 Boss 融化 | 光柱直达战斗区顶边：出膛即中**不需要预判**，全屏射程又让玩家不必靠近，于是站桩对 Boss 的输出是 141.8 发/秒（子弹系同位置约 72/秒） | 给光柱加 `beam_range`（340 像素）与战斗区顶边取小；`PursuitTest` 里有一条取舍断言——**光柱站桩对 Boss 必须打出 0 伤害，子弹系在同一位置必须大于 0**，把射程调回全屏会立刻失败 |
| “按 7 秒设计”的 Boss 5 秒就没了 | 血量按“每秒发数 × 7 秒”反推，公式默认**发发命中**；光柱接近 100% 兑现，算出的 1925 点又被 `boss_hp_cap` 砍到 1400 | 上限抬到 2400（高于任何合法 build 的公式值，只兜底不参与平衡）；`PursuitTest` 断言三台主动上前的击破秒数都落在 5~12 秒 |
| 调平衡时量出“削过头了”，改回去又发现没那回事 | **窗口长度本身会偏向某一种武器**：敌人得先飞进光柱射程，6 秒窗口把“需要敌人靠近”的武器的前半段算成了“没打中” | 同一套配置 6 秒窗口量出 62%/80%/84%（差距 22 点）、10 秒窗口量出 80%/87%/84%（差距 7 点）。**改窗口等于改结论**，基准的采样长度要和结论一起写进注释 |
| 光柱只杀掉“刚进柱”的敌机，停在柱内的反而没事 | 用了 `area_entered`，而它只在进入的那一刻触发一次 | 光柱是持续存在的碰撞体，伤害改为按 `tick_interval` 轮询 `get_overlapping_areas()`；`Monitorable` 关掉无妨，但 `Monitoring` 必须开 |
| Boss 血量在聚焦型下算得偏低 | Boss 血量按“玩家每秒发数”反推，而光柱根本没有“发数”这个量 | `player_dps_proxy()` 为光束单开一条：`车道数 ÷ 结算间隔`；断言比对它与 `lane_count() / beam_tick_interval()` 一致，否则 Boss 会死得比 `boss_target_seconds` 快得多 |
| 第三种形态一加，能力池里出现“选了没变化”的选项 | 穿透弹对光柱本来就是多余的（光柱穿透整列敌人） | 由 `is_upgrade_offered()` 在光束战机上挡掉穿透弹，并配一条对照断言：标准型**仍然**提供它 |
| 调参脚本改了参数却量出逐位相同的结果 | `re.sub(..., count=1)` 命中的是**第一台战机**那一行，而不是要调的第二台 | 改参数后**先把文件里的实际值打出来核对**再跑基准——这条教训这一轮踩了两次 |
| `const` 里写 `PackedVector2Array([...])` 报“不是常量表达式” | GDScript 的 const 不接受该构造函数 | 表里存 `Vector2` 数组字面量，运行时 `PackedVector2Array(...)` 转一次 |
| 三台战机的按钮看不出哪台被选中 | 主题的 pressed 与 normal 样式太接近 | 给按钮单独挂深青底白字的 `pressed`/`hover_pressed` 样式；只靠 ButtonGroup 的默认按下态在截图里看不出区别 |
| 外部评估报了一个“日志里报错”的 P0 | 看到文档里记录的**曾踩过的坑**，误当成现状 | 逐条核对代码与日志：守卫一直在（白名单里只有证书那条，全部日志中该错误零次出现）。**评估结论也要核对，不能照单全收** |
...
| **所有爆炸都画在屏幕左上角** | 生成顺序写成"先 `add_child`、再设坐标"。`Explosion._ready()` 一进来就打开 `emitting`，而 `one_shot + explosiveness=1` 会在**那一瞬间**把整批粒子按当时的坐标（0,0）发射出去——之后再挪节点，粒子已经留在原地了 | 位置必须在 `add_child` **之前**写好（与敌机/Boss 的约定一致）。原来的断言查的是 `burst.global_position`，那个值是对的、所以一直绿着；现在断言查 `Explosion.spawn_position`——**粒子被发射那一刻的位置**。教训与光柱那次同源：**断言的对象必须是玩家看得见的东西** |
...
| 后期难度突然不再上升 | 某个 cap 提前撞顶了（floor 触不到、cap 早就到），而调高 `max_level` 时没回头核对它们 | 每次抬高难度上限都要重新核对所有上下限；文档第 5.10 节记了第一版踩过的具体例子 |
...
| 浸泡基准突然报“残留 Actors=11、对象净增 70” | 战机是写进存档的**持久设置**，而前一条基准（威胁基准）跑完停在聚焦型；光柱是**持续存在的武器本身**，state 还在 PLAYING 时下一帧就会被重建，“停火后残留必须为 0”对光束系根本不成立 | 两条基准都显式 `set_ship("parallel")`，并各带一条“量的是子弹系满配”的守卫断言。**基准必须先声明它量的是哪一台**——否则流程顺序会悄悄改变结论，而日志里的数字看起来毫无异样 |
...
4. 等待导入结束，按 **F5**。
5. 在开始界面先**选一台战机**（标准型 / 游隼型 / 聚焦型，选中态与说明文字会立刻跟着变），再点 **开始飞行**。
6. **WASD/方向键**移动，**按住空格**射击；**Shift** 进低速模式看真实判定范围，**Esc** 暂停与设置。
7. 生命用尽后按 **R** 重新开始；结算页也可以点 **更换战机** 换一台再飞。
