...
```text
Main (Node2D) [Main.gd]
├── Background (ColorRect) [480×800, z_index=-10, Mouse Filter=Ignore]
├── FlightLines (Node2D) [z_index=-9]
│   ├── Left (Line2D) [x=80, y=90..756, width=1]
│   ├── Center (Line2D) [x=240, y=90..756, width=1]
│   └── Right (Line2D) [x=400, y=90..756, width=1]
...
├── LevelUpPanel (Panel) [24,120 → 456,690; 初始 Hidden]
...
**DamageFlash 故意排在 TopBar 之后、各个 Label 之前**：HUD 的子节点按树序绘制，越靠后越在上层。这个位置让红色闪光盖住背景与顶栏色块，却压不住分数、生命、等级这些文字——受伤时画面泛红但信息仍然清楚。它的 alpha 平时为 0，受伤时由 `HUD.flash_damage()` 立刻抬高再补间回落，因此不需要额外的节点开关。

**Boss 血条放在 y=96..116**：这段正好在玩家可达范围（上边界 y=118）之上，所以一条常驻不了几秒的血条既醒目又不会挡住操作区。它平时隐藏，只在 Boss 战期间由 `show_boss()` / `hide_boss()` 开关；`update_boss()` 同时刷新条与旁边的 `BOSS n/m` 文字。
...
面板按“基础 4 个、幸运补给后 5 个”预留了 5 张卡片：卡片高 82、间距 12，从 y=206 排到 664，正好落在面板 120..690 之内；`show_level_up()` 只显示实际给出的张数，多余的隐藏，因此 4 选 1 时不会留下点不动的空格子。这些几何关系在回归测试里有断言（文案不超出卡宽、第 5 张不越出面板下沿、首张不压住标题），加能力或改字号会立刻被发现。
...
**`res://tests/ThreatTest.gd` 是威胁基准，量“后期还有没有威胁”**。它要回答的问题很具体：**敌方的火力有多少真的能穿过玩家弹幕、到达玩家面前**。指标有两个——**敌弹通过率**与**每架敌机开了几枪**——后者是核心诊断量。玩家固定在屏幕下方且全程不动，所以通过率与躲闪技术无关；同时它也是“最差情况”的命中数。这条基准的由来是一次方向性错误：真人反馈“一点威胁都没有”，而我此前几轮一直在**敌人那一侧**加码（密度、速度、开火概率、瞄准比例、难度曲线），而基准量出的真相是**每架敌机平均只开 0.69 枪**（按存活时间本该 2~4 枪）——玩家每秒 148 发、11 条弹道，把敌机在开火窗口之前就打死了，那些参数乘的都是“敌机能开几枪”，而它已经被压到接近 0。修法是齐射（见第 1.1 节）。它同样把结论固化成了断言：最高难度下每架敌机 ≥ 1.5 发、每秒 ≥ 8 发到达，一旦威胁再次被抹平就会失败。**注意：它必须把 `xp_multiplier` 设为 0**，否则击毁敌机会触发升级抉择把整树暂停，`_create_enemy` 直接返回，量出来的“通过率 0%”是一个由故障伪装成的结论（这个坑第一版就踩了）。
