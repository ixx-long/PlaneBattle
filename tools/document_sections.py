...
- **开局选战机**：开始界面显示可选战机，选定后整局沿用，并记进存档（下次启动仍是它）。**形态刻意不进抽卡池**——不稀释 16 项能力的抽取，也不会出现"什么都拿一点"导致覆盖形状与输出同时膨胀；同时它避开了"中途改武器会让前几级投资作废"的问题。当前三台：
  ...
  - **聚焦型**：**贯穿光束**，出膛即中——光柱（`scenes/Beam.tscn`）是一条持续存在的竖直致命线，敌机一进柱就在下一次结算时被击毁，**没有飞行时间**。
  三台刻意**只改武器行为与外形**，不碰移速/生命/冷却：每台各换一套武器，需要验证的组合已经翻倍，再叠加数值取舍，出问题时就分不清是形态还是数值造成的。
- **光束的代价是覆盖，而它反过来是“最没有威胁”的那一台**：每条光柱只有 `beam_width`（默认 10 像素）宽，柱外的敌机完全不受影响——这是它唯一的代价。但“出膛即中”把敌机的存活时间压到了最短，所以威胁基准里它的敌弹到达量反而比标准型低（17.00 对 19.83 发/秒，见第 10 节）。**平衡支点就是 `beam_width`**：光柱越宽，被瞬间清掉的敌机越多、敌方弹幕越少。这里的取舍和追踪导弹是同一个道理——武器越“能杀”，敌人越没机会还手，所以每台战机的形状都必须量过才准上。
...
**它与其余实体的本质区别是：它持续存在，而且几何每帧都在变。** 子弹是“生成 → 飞 → 命中或出界 → 自己释放”，光柱则从开局起一直在屏幕上，长度随玩家上下移动而伸缩（`Main._sync_beams()` 每帧把 `player.global_position.y - 34 - play_area_top` 送进来），条数与横向位置则**照抄子弹车道的算法**——`_lane_offsets()` 是子弹与光柱共用的唯一来源，所以“火力增援/侧翼炮”在这台战机上含义完全相同，两台战机的差别只剩“瞬间命中 vs 飞行时间”和“覆盖窄 vs 弹幕墙”。
