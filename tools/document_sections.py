...
- **开局选战机**：开始界面显示可选战机，选定后整局沿用，并记进存档（下次启动仍是它）。**形态刻意不进抽卡池**——不稀释 16 项能力的抽取，也不会出现"什么都拿一点"导致覆盖形状与输出同时膨胀；同时它避开了"中途改武器会让前几级投资作废"的问题。当前两台：
  - **标准型**：平行弹幕，基线。
  - **游隼型**：追踪弹。子弹向**近距离**（110 像素内）的最近敌机转向；更远的敌机不锁定，子弹照直飞。
  第一版刻意**只改武器行为与外形**，不碰移速/生命/冷却：两台战机各自换一套武器，需要验证的组合已经翻倍，再叠加数值取舍，出问题时就分不清是形态还是数值造成的。
- **追踪弹的平衡支点不是命中率，而是射程**：敌机是朝玩家下来的，所以只要射程不受限，追踪一定撞得上——用转向速率压不住它（实测把转向降到 1.0 仍无济于事，击毁率 92~96%）。真正决定威胁的是**锁定射程**：满配每秒 148 发、在飞约 170 发，弹群密度极高，射程一放开就会把上半屏的敌机全部清掉，敌方弹幕被打到 0。实测曲线（威胁基准在难度 30 的"到达/秒"，门槛 12）：射程 70 → 15.17、**110 → 15.33**、300 → 6.50、600 → 5.17、不限制 → ~0。断点很陡，所以这个数字是平衡参数而不是手感参数。它同时也是这台战机的身份：**打不到远处，只能近程拦阻**。
...
`homing` 与 `homing_turn_rate` 由 Main 按当前战机设定（游隼型启用）。转向时子弹会同时旋转图形，玩家因此看得出"这发在拐弯"；平行弹幕时 `direction` 恒为正上方、旋转量恒为 0，与旧行为逐值一致。**目标来自一个共享的组查询**（`get_first_node_in_group("player_target")`），而不是每颗子弹各自遍历敌机——满配每秒 148 发，逐弹遍历会变成每秒几千次全表搜索。这个写法与 `Enemy.gd` 瞄准玩家时用 `get_first_node_in_group("player")` 是同一个套路。
...
| 追踪弹把威胁整个抹平 | 敌机朝玩家下来，射程不受限时追踪必然命中；弹群密度又高（满配在飞约 170 发） | 平衡支点是**锁定射程**而不是转向速率：110 像素内才锁，实测到达/秒 15.33 达标；射程 300 掉到 6.50 |
