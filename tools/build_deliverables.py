...
- **建立了一条“威胁基准”并据此修掉了真正的病因：威胁是在源头被掐死的。** 真人反馈“一点威胁都没有，玩家强度升起来太快”，而我此前几轮一直在**敌人那一侧**加码（密度、速度、开火概率、瞄准比例、难度曲线），方向就错了。新基准量的是两件事：**敌弹通过率**（敌方火力有多少真的穿过玩家弹幕到达玩家面前）与**每架敌机开了几枪**。基线数据（满配 build、玩家全程不动）：难度 30 时 6 秒只生成 45 发敌弹、**每架敌机平均只开 0.69 枪**（按存活时间本该 2~4 枪），通过率却有 65~80%。也就是说拦截弹**不是**瓶颈（我最初怀疑错了），真正的瓶颈是：**玩家每秒 148 发、11 条弹道，把敌机在开火窗口之前就打死了**——密度、速度、开火概率乘的都是“敌机能开几枪”，而那个数已经被压到接近 0，所以怎么调都没用。修法是**齐射**（`volley_every_levels` / `volley_cap` / `volley_spread_degrees`）：难度每高 10 级，敌机一次多打一发（最高 3 发，左右对称散开），把总输出改成由**首发**决定，绕开“活多久”这个被玩家火力支配的变量。修复后同一基准：每架 **{threat_shots_per_enemy}** 发、每秒 **{threat_arrivals}** 发到达（改前 0.69 发 / 3.33 发），通过率 {threat_pass_rate}%。该指标已固化成断言（最高难度下每架 ≥ 2.5 发、每秒 ≥ 12 发到达），以后再出现“没威胁”会直接失败。**这里还补了一个方法论修正**：门槛原来是 1.5 / 8.0，理由是“容忍抖动”，但干净环境复验时偶发失败——实测最小值恰好是 8.17，下限压在了边缘上。根子不是门槛而是基准本身波动太大（“到达/秒”在 8~19 之间跳，因为出生位置、开火与瞄准判定都走随机数）。于是给基准**固定随机种子**：连跑三次变成 19.83 / 20.33 / 19.83，只剩帧时序带来的零点几。门槛这才敢收回到 2.5 / 12.0——**一个守不住东西的下限等于没有下限**。
