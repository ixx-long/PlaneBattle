...
# 结果都取出来——文档要能反映"三台战机各有各的量值"，而不是只报一台。
...
- 已实现并验证第 9 节的音频增强：**程序合成的音效与循环背景音乐**。**八个** WAV（合计约 508 KB，其中含 Boss 登场警告音与**转阶段音效**）由 `tools/generate_audio.py` 只用 Python 标准库生成，固定随机种子、逐字节可复现，因此没有第三方素材与授权问题；音乐走 Music 总线、音效走 SFX 总线（`default_bus_layout.tres`），后者由 6 路音效池播放以避免连射时后一发掐断前一发。
...
- 已执行真实图形渲染，检查开始、战斗、暂停与设置、爆炸特效、**追踪导弹**、**贯穿光束**、**Boss 战**、升级抉择、结算九种界面截图。战斗、追踪与光束截图由可复现测试布置敌机/子弹并切换战机后拍摄（光束那张同时给出“柱内即死、柱外无事”的视觉证据），升级截图经真实升级路径触发面板，Boss 截图走真实登场路径并把机体推到位，特效截图特意关掉震动以便看清爆炸本身。以上均用于视觉验收，不是玩家实际战绩。音频为程序合成，听感接近 8-bit，属于可用的占位素材而非成品音源。
...
- **建立了一条“威胁基准”并据此修掉了真正的病因：威胁是在源头被掐死的。** 真人反馈“一点威胁都没有，玩家强度升起来太快”，而我此前几轮一直在**敌人那一侧**加码（密度、速度、开火概率、瞄准比例、难度曲线），方向就错了。新基准量的是两件事：**敌弹通过率**（敌方火力有多少真的穿过玩家弹幕到达玩家面前）与**每架敌机开了几枪**。基线数据（满配 build、玩家全程不动）：难度 30 时 6 秒只生成 45 发敌弹、**每架敌机平均只开 0.69 枪**（按存活时间本该 2~4 枪），通过率却有 65~80%。也就是说拦截弹**不是**瓶颈（我最初怀疑错了），真正的瓶颈是：**玩家每秒 148 发、11 条弹道，把敌机在开火窗口之前就打死了**——密度、速度、开火概率乘的都是“敌机能开几枪”，而那个数已经被压到接近 0，所以怎么调都没用。修法是**齐射**（`volley_every_levels` / `volley_cap` / `volley_spread_degrees`）：难度每高 10 级，敌机一次多打一发（最高 3 发，左右对称散开），把总输出改成由**首发**决定，绕开“活多久”这个被玩家火力支配的变量。修复后同一基准：每架 **{threat_shots_per_enemy}** 发、每秒 **{threat_arrivals}** 发到达（改前 0.69 发 / 3.33 发），通过率 {threat_pass_rate}%。该指标已固化成断言，且**每台战机都必须达标**（最高难度下每架 ≥ 2.5 发、每秒 ≥ 12 发到达）——'
 '形态把平衡面翻了一倍多，靠手调守不住，所以把它变成构建级断言。三台当前实测：{threat_per_ship}。**这里还补了一个方法论修正**：门槛原来是 1.5 / 8.0，理由是“容忍抖动”，但干净环境复验时偶发失败——实测最小值恰好是 8.17，下限压在了边缘上。根子不是门槛而是基准本身波动太大（“到达/秒”在 8~19 之间跳，因为出生位置、开火与瞄准判定都走随机数）。于是给基准**固定随机种子**：连跑三次变成 19.83 / 20.33 / 19.83，只剩帧时序带来的零点几。门槛这才敢收回到 2.5 / 12.0——**一个守不住东西的下限等于没有下限**。
...
- **这一轮最重要的教训："能量准"与"看得见"不是一回事。** 威胁基准把平衡量得很准（每项改动都有数字），但它**量不出玩家能不能察觉到这个功能**——110 像素的锁定在数据上完全达标，在真人眼里却是"功能没做出来"。基准能替代"靠感觉调参"，替代不了"靠眼睛验收"。同类的还有：真人反馈"选择机型时说明文字没变"——那条界面刷新缺陷，所有断言都碰不到。
- 期间修掉三个**我自己引入**的缺陷：① 锁定指示器在无目标时被摆到玩家正上方当替身，导致两侧车道的子弹全朝中间拐、弹幕从"一排平行车道"变成"一束"（症状是参数怎么改都量出**逐位相同**的结果，说明它根本没参与）；② 换战机后开始界面的说明文字不刷新（`ship_detail` 只在 `show_start()` 里赋过值），玩家会以为没点到；③ 战机按钮只靠 `ButtonGroup` 的默认按下态，截图里两台看起来一模一样，改挂深青底白字的 `pressed` 样式才可辨。另外 `const` 里不能写 `PackedVector2Array([...])`（不是常量表达式），改为存 `Vector2` 字面量、运行时转换。
- **威胁基准因此扩成了遍历全部战机**：形态把平衡面翻了一倍多，靠手调守不住，所以"每台战机都必须达标"变成了构建级断言——游隼型第一次跑出来就是 `每架开火 0.00 / 到达每秒 0.00`，直接被拦下；聚焦型（光束）则是首轮通过、量值 17.00 发/秒。这正是把主观感受变成可回归指标的价值。
...
- **给 Boss 加了二阶段：换的是弹幕，不是速度。** 血量掉到 50% 时停住横向巡航，扇形齐射改成**螺旋弹**——每轮把若干发弹均分整圈射出、每轮整体再转一个固定角，所以单看一轮只是个正多边形、把相邻几轮叠起来才看得出"在转"；发射点是机体中心（径向弹幕从核心冒出来才像转），弹速刻意比扇形慢，留出"找缝隙钻"的时间。转阶段有 0.5 秒的仪式：**全屏闪白 + 屏幕震动 + 专属音效**（上行的抖颤锯齿，与登场的下行音效正好构成一对），血条文字与配色同时变暖。**"停一下、闪一下、换弹幕"是刻意的**：不声不响地换一套弹幕，玩家只会以为弹幕变密了，而不会觉得"它换了打法"。实现上：Boss 只广播"我换阶段了"，仪式全在 Main/HUD 一侧（与敌机爆炸同一条约定）；阶段只升不降，因此整场只会播一次仪式。回归新增 16 条断言（触发边界、仪式期间停火停巡航、三件事同时发生、螺旋均分整圈且每轮转过的角度等于设定步长、覆盖整圈有朝上的弹、标记不被覆盖）。**其中一条是被截图抓出来的**：血条上的"二阶段"标记原本一挨打就被血量刷新冲掉，而当时断言全绿——"标了字"与"字一直在"是两回事，这已经是本项目第三次被截图纠正。
- **这轮的改动也顺手补了一个潜在泄漏**：`EnemyBullet` 的出界判定原本只判下边与左右，螺旋弹里有朝正上方飞的那几发，它们会一直往上飞、永远不释放。现在四个方向都判。
...
for filename, caption in [
    ...
    ('boss-phase-preview.png', '09 / Boss 转阶段：全屏闪白 + 血条换色'),
    ('boss-spiral-preview.png', '10 / Boss 二阶段：螺旋弹幕'),
    ('levelup-preview.png', '11 / 升级抉择界面'),
    ('gameover-preview.png', '12 / 结算界面'),
]:
