...
## **这个数是量出来的**：10 秒窗口下三台实测 80% / 87% / 84%，差距 7 个点（连跑三次一致），
## 所以上限取 18——是实测值的两倍半，既拦得住"某台一家独大"，又不会被帧时序抖动掀翻。
## 注意窗口长度会直接影响这个数：同样的配置在 6 秒窗口里量出 22 个点，因为敌人得先飞进
## 光柱的射程，短窗口把"需要敌人靠近"的武器的前半段算成了"没打中"。
const MAX_KILL_SPREAD: float = 0.18
	...
	for ship in game.SHIPS:
		...
		if bool(standing_boss["ok"]):
			...
			if game.ship_uses_beam():
				...
				check(
					int(standing_boss["dealt"]) > 0,
					"%s：同位置上子弹系照样能打到 Boss（对照：这是光束独有的取舍）" % ship["name"]
				)
