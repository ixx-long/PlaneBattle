...
## 缺陷：玩家火力把敌机在开火之前就打死，于是敌方总输出被抹平——当时实测每架只开
## 0.69 发、每秒只有 3.33 发到达。补上齐射后实测为 2.1~3.3 发/架、12.2~17.5 发/秒
## （四次运行）。所以 1.5 发/架与 8 发/秒既能容忍随机出生位置带来的抖动，
## 又能在威胁再次被抹平时立刻失败。
const MIN_SHOTS_PER_ENEMY: float = 1.5
const MIN_ARRIVALS_PER_SECOND: float = 8.0
